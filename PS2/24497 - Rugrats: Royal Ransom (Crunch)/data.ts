import { define as $, Condition, ConditionBuilder } from '../node_modules/@cruncheevos/core'
...
export const levelIDInstant = [ 'Mem',  '8bit', 0x3d277c]

// $3F64DC: [8-bit] Level ID, currently loaded level
export const levelIDLoaded = [ 'Mem', '8bit', 0x3F64DC ]
...
export const currentNumberOfPlayers = ['Mem',  '8bit', 0x3F64F0 ]

// $3F64F8: [32-bit] Current funny money
//          Starts at 0x0 at the start of a save file
export const currentFunnyMoney = [ 'Mem', '32bit', 0x3F64F8 ]

// $3F64FC: [32-bit] Current coins
//          Starts at 0x01f4 at the start of a save file
export const currentCoins = ['Mem', '32bit', 0x3F64FC]

// $3F6500: [32-bit] Current big batteries
//          Starts at 0x0 at the start of a save file
export const currentBigBatteries = [ 'Mem',  '32bit', 0x3F6500 ]

// $3F6504: [32-bit] Currnet small batteries
//          Starts at 0x0 at the start of a save file
export const currnetSmallBatteries = ['Mem',  '32bit',  0x3F6504] 

// $3F6508: [8-bit] Difficulty of last loaded game
//          0x00 - Baby Easy
//          0x01 - Rugrat Normal
//          0x02 - Reptar Tough
export const difficulty = [ 'Mem', '8bit', 0x3F6508 ] 
...
//          0x00 - No
//          0x01 - Yes
export function levelUnlocked(levelID:number):Array { return [ 'Mem', '32bit',  0x3F6514 + 4 * (levelID - 1) ] }
...
export function bigBatteryCollected(levelID: number): Array  { return [  'Mem', '32bit',  0x3F65D8 + 4 * (levelID - 1) ] }
...
export function shopItemAvailable(levelID: number): Array  { return [ 'Mem',  '32bit', 0x3F6698 + 4 * (levelID - 1) ] }
...
export function shopItemPurchased(levelID: number): Array  { return [ 'Mem',  '32bit',  0x3F672C + 4 * (levelID - 1) ] }
...
export function littleBatteriesLeft(levelID: number): Array  { return [ 'Mem',  '16bit',  0x3F98C2 + 2 * (levelID - 1) ] }
...
export function funnyMoneyLeft(levelID: number): Array  { return [ 'Mem',  '16bit',  0x3F9924 + 2 * (levelID - 1) ] }
...
export function chainLinkedListData(node: number, ifForward: boolean = true, accessNode: boolean = true): ConditionBuilder {
    ...
    let chain: ConditionBuilder = $(
        $(['Add Address']).with(lvalue = mainPointer),
        ifForward && ['Add Address', 'Value', '', 0x10],
        !ifForward && ['Add Address', 'Value', '', 0x44],
        !ifForward && ['Add Address', 'Value', '', 0x30]
    )
    ...
    while (i < node) {
        chain = $(
            chain,
            ifForward && ['Add Address', 'Value', '', 0x14],
            !ifForward && ['Add Address', 'Value', '', 0x10]
        )
        i = i + 1
    }

    return $(chain, accessNode && ['Add Address', 'Value', '', 0x0])
}
...
export function chainLinkedListDataRange(startnode: number, endnode: number, conditions: Array<ConstructionBuilder>, ifForward: boolean = true): ConditionBuilder {
    ...
    const nextRowForward = $(['Add Address', 'Recall'], ['Remember', 'Value', '', 0x14])
    const nextRowBackward = $(['Add Address', 'Recall'], ['Remember', 'Value', '', 0x10])
